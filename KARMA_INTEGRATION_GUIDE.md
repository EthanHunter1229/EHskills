...
3. 复制 Skill 提示词:
   - 打开 formal-report-formatting/KARMA_SKILL.md
...
| 方式 | 速度 | 灵活性 | 离线支持 | 推荐场景 |
|------|------|--------|----------|----------|
| GitHub 克隆 | 5星 | 5星 | 支持 | 开发、批量处理、高频使用 |
| 手动导入 | 3星 | 4星 | 不支持 | 集成对话、团队共享 |
| 自动创建 | 1星 | 3星 | 不支持 | 懒人模式、一次性使用 |
...
# 快速排版(正则模式,1秒完成)
python3 scripts/format_docx.py   ~/Downloads/招股说明书.docx   ~/Downloads/招股说明书_排版.docx   --rules assets/default_rules.json
```
...
```bash
# 智能识别(LLM 模式,30秒完成)
python3 scripts/format_docx.py   ~/Downloads/企业年报.docx   ~/Downloads/企业年报_排版.docx   --use-llm   --rules assets/default_rules.json
```
...
正则模式误判太多? → 使用 LLM 模式: --use-llm
LLM 模式也不准? → 检查文档结构,可能需要手动标注标题样式
